...
(() => {
    ...
    const UNAUTHENTICATED_ACCESS_TOKEN_PATH =
        IN_APP_API_PREFIX +
        IN_APP_API_VERSION +
        AUTHORIZATION_PATH +
        UNAUTHENTICATED_PATH +
        ACCESS_TOKEN_PATH;
    const AUTHENTICATED_ACCESS_TOKEN_PATH =
        IN_APP_API_PREFIX +
        IN_APP_API_VERSION +
        AUTHORIZATION_PATH +
        AUTHENTICATED_PATH +
    ...
    const CONVERSATION_PATH =
        IN_APP_API_PREFIX + IN_APP_API_VERSION + "/conversation";
    const CONTINUITY_ACCESS_TOKEN_PATH =
        IN_APP_API_PREFIX +
        IN_APP_API_VERSION +
        AUTHORIZATION_PATH +
        "/continuityAccessToken";
    const LIST_CONVERSATIONS_PATH =
        IN_APP_API_PREFIX +
        IN_APP_API_VERSION +
        QUERIES_PATH +
        "/conversation/list";
    const REGISTER_DEVICE_CAPABILITIES_PATH =
        IN_APP_API_PREFIX + IN_APP_API_VERSION + CAPABILITIES_PATH;
    ...
    function handleCreateNewConversation() {
        ...
        return createNewConversation().then((conversationResponse) => {
            ...
            console.log(
                `Initialized the esDeveloperName associated with current conversation to: ${agentforce_messaging.settings.eswConfigDevName}`
            );
    ...
    function createNewConversation(routingAttributes) {
        // TODO: snippetConfig
        const endpoint =
            agentforce_messaging.settings.snippetConfig.scrt2URL.concat(
                CONVERSATION_PATH
            );

        return sendFetchRequest(endpoint, "POST", "cors", null, {
    ...
    function getUnauthenticatedJwt() {
        ...
        const endpoint =
            agentforce_messaging.settings.snippetConfig.scrt2URL.concat(
                UNAUTHENTICATED_ACCESS_TOKEN_PATH
            );
    ...
    function sendFetchRequest(
        ...
        const headers = requestHeaders
            ...
            : {
                  "Content-Type": "application/json",
                  ...(messagingJwt && {
                      Authorization: "Bearer " + messagingJwt
                  })
              };
        ...
        return fetch(apiPath, {
            ...
            const timeElapsed = (
                ...
                1000
            ).toFixed(3); // Read the timeElapsed in seconds and round the value to 3 decimal places.
            console.log(
                "sendFetchRequest",
                `${caller} took ${timeElapsed} seconds and returned with the status code ${response.status}`
            );
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    responseObject = Object.assign(response, {
                        message: `Error reading the body stream of error object: ${response}`
                    });
                }
